...
export async function GET(request: NextRequest) {
  ...
  await ensureSchema()
  const db = getDb()
  const result = await db.execute('SELECT * FROM restock_requests ORDER BY created_at DESC')
...
export async function POST(request: NextRequest) {
  ...
  await db.execute({
    sql: `INSERT INTO restock_requests (id, nickname, description, image_data, status, created_at)
          VALUES (?, ?, ?, ?, 'pending', ?)`,
...
export async function PATCH(request: NextRequest) {
  ...
  const now = new Date().toISOString()
  await db.execute({
    sql: `UPDATE restock_requests SET status = 'noted', noted_at = ? WHERE id = ?`,
    args: [now, body.id],
  })
