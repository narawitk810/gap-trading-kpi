...
export async function GET(request: NextRequest) {
  ...
  if (date) {
    const [sales, gifts] = await Promise.all([
    ...
    return NextResponse.json({ date, nickname, sales: sales.rows, gifts: gifts.rows })
  }
  ...
  if (month) {
    ...
    return NextResponse.json({ month, nickname, sales: sales.rows, gifts: gifts.rows })
  }
...
export async function POST(request: NextRequest) {
  ...
  const { nickname, date, sales = [], gifts = [] } = body
  ...
  return NextResponse.json({ ok: true, inserted: { sales: sales.length, gifts: gifts.length } })
}
