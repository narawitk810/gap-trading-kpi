...
export async function GET(request: NextRequest) {
  ...
  if (adminId) {
    ...
    const row = await db.execute({ sql: 'SELECT id, department, title, content, image_data, image_name, file_data, file_name, created_by FROM dept_announcements WHERE id = ?', args: [adminId] })
