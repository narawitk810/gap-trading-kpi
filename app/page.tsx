...
const DEPT_GROUPS = [
  ...
  { label: 'กองหลัง', icon: '🛡️', depts: ['ธุรการ', 'บัญชี&การเงิน', 'แพค', 'บุคคล', 'ผู้จัดการไลฟ์สด', 'ผู้จัดการหน้าร้าน', 'กฎหมาย', 'Audit'] },
] as const
...
export default function Home() {
  ...
  async function handleConfirm() {
    ...
    if (formData.department === 'ธุรการ' && extra_data) {
      extra_data.checklist = thurakarnChecklist
    }
    try {
  ...
  return (
    <div className="min-h-screen bg-[#F5F6F8]">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5 pb-10">
        ...
        {['บัญชี&การเงิน', 'ธุรการ', 'บุคคล', 'การตลาด', 'Creative', 'Sales Admin'].includes(formData.department) && (
          <Link
            href="/disbursement"
            className="flex items-center gap-3 bg-orange-50 border border-orange-200 rounded-2xl p-4 hover:bg-orange-100 transition-colors"
          >
        ...
        {formData.department === 'ไลฟ์สด' && (
          ...
          >
            ...
            <div>
              <p className="text-sm font-bold text-[#DC2626]">แจ้งรายชื่อ Blacklist ลูกค้า</p>
              <p className="text-xs text-gray-400 mt-0.5">บันทึกลูกค้าที่ต้องระวัง พร้อมรูปหลักฐาน</p>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-[#DC2626] ml-auto shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
        ...
        {formData.department === 'Sales Admin' && (
          <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-[#374151] mb-2">ยอดขายวันนี้</label>
        ...
        {formData.department === 'แพค' && (
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            ...
            <div className="space-y-2.5">
              {([
                { key: 'restockSchedule', label: 'เติม stock ทุกๆ 11.30 น. และ 16.30 น. ของทุกวัน' },
                { key: 'cleanWarehouse', label: 'ทำความสะอาดคลังสินค้าสม่ำเสมอ' },
                { key: 'organizeShelf', label: 'จัดชั้นวางให้สวยงาม' },
              ] as { key: keyof typeof packChecklist; label: string }[]).map(({ key, label }) => (
                <label key={key} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={packChecklist[key]}
                    onChange={(e) => setPackChecklist(prev => ({ ...prev, [key]: e.target.checked }))}
                    className="w-5 h-5 rounded border-[#E2E8F0] accent-[#1E3A5F] cursor-pointer"
                  />
                  <span className="text-sm text-[#374151]">{label}</span>
                </label>
        ...
        {formData.department === 'ธุรการ' && (
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            ...
            <div className="space-y-2.5">
              {([
                { key: 'storeDocs', label: 'จัดเก็บเอกสาร' },
                { key: 'checkDocInOut', label: 'ตรวจเอกสารเข้า-ออก' },
                { key: 'followUpDocs', label: 'ติดตามเอกสารค้างแต่ละแผนก' },
                { key: 'checkEquipment', label: 'ตรวจอุปกรณ์สำนักงาน' },
                { key: 'dailyExpenses', label: 'รวมรายจ่ายทั้งหมดในบริษัทต่อวัน' },
              ] as { key: keyof typeof thurakarnChecklist; label: string }[]).map(({ key, label }) => (
                <label key={key} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={thurakarnChecklist[key]}
                    onChange={(e) => setThurakarnChecklist(prev => ({ ...prev, [key]: e.target.checked }))}
                    className="w-5 h-5 rounded border-[#E2E8F0] accent-[#1E3A5F] cursor-pointer"
                  />
                  <span className="text-sm text-[#374151]">{label}</span>
                </label>
            ...
            {errors.thurakarnChecklist && (
              <p className="text-[#DC2626] text-xs mt-2">{errors.thurakarnChecklist}</p>
            )}
          </div>
        )}
        ...
        {/* Obstacles — hidden for ไลฟ์สด (handled in custom card above) */}
        {formData.department !== 'ไลฟ์สด' && (
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <label className="block text-sm font-semibold text-[#374151] mb-2">
              อุปสรรคที่เจอวันนี้
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              {formData.department === 'Sales Admin' && extra.salesAmount && (
                <div className="pt-1 border-t border-[#E2E8F0]">
                  <ConfirmRow label="ยอดขาย" value={`${Number(extra.salesAmount).toLocaleString()} บาท`} />
                </div>
              )}
